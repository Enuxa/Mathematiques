...
\usepackage{graphicx}
\usepackage{geometry}
\usepackage[]{algorithm2e/tex/algorithm2e}
...
\begin{algorithm}
	...
	\While{$var(S) \neq \emptyset$ et $\square \notin S$}{
		choisir $p \in var(S)$ \\
		$S \leftarrow S \backslash \{C \in S ~ | ~ p \in C \text{ et } \neg p \in C \}$ \\
		$Res \leftarrow \{C \cup P ~ | ~ C \cup \{p\} \in S \text{ et } D \cup \{\neg p\} \in S \}$ \\
		$S \leftarrow Res \cup \{C \in S ~ | ~ p \notin C \text{ et } \neg p \in C \}$ \\
	}
...
\begin{algorithm}
	$S \leftarrow \varphi$ \\
	\For{$q \in var(\varphi)$}{$v(q) \leftarrow 0$}
	\caption{DP-Resolution-Horn($\varphi$)}
	\While{$unit(S) \neq \emptyset$ et $\square \notin S$}{
		choisir $\{l\} \in unit(\varphi)$ \\
		$v(l) \leftarrow 1$ \\
		$S \leftarrow S \backslash \{C \in S ~ | ~ l \in C \land \neg l \in C \}$ \\
		$Res_p \leftarrow \{C ~ | ~ C \cup \{\neg p\} \in S \}$\\
		$S \leftarrow \{C ~ | ~ C \cup \{\neg p\} \in S \}$
	}
	\uIf{$\square \in S$}{\Return 0}
	\Else{\Return $v$}
\end{algorithm}
...
\begin{algorithm}
	...
	\uElseIf{$unit(\varphi) \neq \emptyset$}{
		Choisir $l \in unit(\varphi)$ \\
		\Return DPLL($\varphi[l/\top]$) \\
	}
	\uElseIf{$Pol^+(\varphi) \cup Pol^-(\varphi) \neq \emptyset$}{
		Choisir $x \in Pol^+(\varphi)$ \\
		Choisir $y \in Pol^-(\varphi)$ \\
		\Return DPLL($\varphi[x/\top][y/\bot]$)
	}
	\Else{
		Choisir $x \in var(\varphi)$ \\
		\Return $DPLL(\varphi[x/\top])$ ou $DPLL(\varphi[x/\bot])$
	}
