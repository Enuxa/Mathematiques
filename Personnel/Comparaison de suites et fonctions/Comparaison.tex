...
\begin{enumerate}
	...
	$f_n$ est dérivable sur $\mathbb{R}_+$, de dérivée $f'_n(x)=n (2x e^{-x\sqrt{n}}-\sqrt{n} \, e^{-x\sqrt{n}}x^2)=nxe^{-x\sqrt{n}}(2-x\sqrt{n})$
	
	$||f_n||_{\infty, \mathbb{R}_+}=\left|f_n\left(\frac{2}{\sqrt{n}}\right)\right|=\frac{4}{e^2}$
	...
	Or comme on l'a vu en (a), $f_n(a)=O(n^{-3})$, donc $\displaystyle \sum f_n$ converge uniformément sur $A$.
	
	\item Si $\sum f_n$ convergeait uniformément sur $\mathbb{R}_+$, nécessairement $\|f_n\|_{\infty, \mathbb{R}_+} \longrightarrow 0$, ce qui est faux d'après (b).
	
	\item Chaque $f_n$ est continue et donc $\displaystyle \sum_{k=1}^{n} f_n$ est continue sur $A$, d'où $h$ est continue sur $A$ par la convergence uniforme de la série.
\end{enumerate}
